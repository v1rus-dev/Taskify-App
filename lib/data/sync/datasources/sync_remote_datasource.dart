...
import 'package:taskify/data/sync/models/sync_push_request_model.dart';
...
abstract class SyncRemoteDataSource {
  Future<Either<Failure, SyncPushResponseModel>> pushChanges({
    ...
    required List<SyncOpInputModel> ops,
  });

  Future<Either<Failure, SyncChangesModel>> getChanges({
    required int cursor,
    int limit,
    bool compact,
  });
...
class SyncRemoteDataSourceImpl implements SyncRemoteDataSource {
  ...
  Future<Either<Failure, SyncPushResponseModel>> pushChanges({
    ...
    TalkerService.instance.info(
      'syncTag pushChanges start: ${ops.length}',
    );
  ...
  Future<Either<Failure, SyncChangesModel>> getChanges({
    ...
    final result = await _client.get<SyncChangesModel>(
      path: '/sync/changes',
      queryParameters: {
        'cursor': cursor,
        'limit': limit,
      ...
      parser: (data) =>
          SyncChangesModel.fromJson(data as Map<String, dynamic>),
    );
    result.fold(
      ifLeft: (failure) =>
