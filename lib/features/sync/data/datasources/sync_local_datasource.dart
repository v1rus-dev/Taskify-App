...
import 'package:taskify/core/database/tables/sync_state_table.dart';
...
class SyncLocalDataSourceImpl implements SyncLocalDataSource {
    ...
    try {
      ...
      final ops = rows.map((row) {
        ...
        return SyncQueueEntryModel(
          ...
          data: payload == null
              ? null
              : SyncOpDataRequestModel.fromJson(payload),
        );
      }).toList();
  ...
  Future<Either<Failure, void>> removeOpsByOpIds(List<String> opIds) async {
    try {
      ...
      )..where((row) => row.opId.isIn(opIds))).go();
  ...
  Future<Either<Failure, SyncStateModel>> getState() async {
    try {
      ...
      final row =
          await (_database.select(_database.syncStateTable)
      ...
      TalkerService.instance.info(
        'syncTag getState result: ${state.lastCursor}',
      );
      return Right(state);
    } catch (e) {
      ...
      await _database.transaction(() async {
        for (final entry in taskMap) {
          await (_database.update(_database.tasksTable)
              ...
              .write(TasksTableCompanion(networkId: Value(entry.id)));
        }
        for (final entry in subTaskMap) {
          await (_database.update(_database.subtasksTable)
                ..where((subtask) => subtask.clientId.equals(entry.clientId)))
              .write(SubtasksTableCompanion(networkId: Value(entry.id)));
        }
      });
  ...
  Future<void> _applyTaskChange(SyncEventItemResponseModel change) async {
    ...
    if (existing == null && clientId != null) {
      existing = await (_database.select(
        _database.tasksTable,
      )..where((task) => task.clientId.equals(clientId))).getSingleOrNull();
    }
  ...
  Future<void> _applySubTaskChange(SyncEventItemResponseModel change) async {
    ...
    final localTaskId = await _resolveLocalTaskId(
      taskId: taskId,
      taskClientId: taskClientId,
    );
    if (localTaskId == null) {
      TalkerService.instance.warning('syncTag subtask skipped: task not found');
      return;
    }

    var existing =
        await (_database.select(_database.subtasksTable)
    ...
    if (change.op == 'delete') {
      ...
      )..where((subtask) => subtask.id.equals(existing!.id))).write(
  ...
  Future<int?> _resolveLocalTaskId({int? taskId, String? taskClientId}) async {
    if (taskId != null) {
      final task = await (_database.select(
        _database.tasksTable,
      )..where((row) => row.networkId.equals(taskId))).getSingleOrNull();
      return task?.id;
    }
    if (taskClientId != null) {
      final task = await (_database.select(
        _database.tasksTable,
      )..where((row) => row.clientId.equals(taskClientId))).getSingleOrNull();
      return task?.id;
    }
    return null;
  }
