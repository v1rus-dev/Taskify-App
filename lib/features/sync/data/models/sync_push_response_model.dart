...
import 'package:taskify/features/sync/data/models/sync_op_error_item_response_model.dart';
  ...
  factory SyncPushResponseModel.fromJson(Map<String, dynamic> json) {
    ...
    for (final entry in idMapRaw.entries) {
      mappedIdMap[entry.key] = (entry.value as List<dynamic>)
          .map(
            (item) => SyncIdMapItemResponseModel.fromJson(
    ...
    return SyncPushResponseModel(
      ...
      errors: (json['errors'] as List<dynamic>? ?? [])
          .map(
            (item) => SyncOpErrorItemResponseModel.fromJson(
              item as Map<String, dynamic>,
            ),
          )
          .toList(),
