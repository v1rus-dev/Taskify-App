...
class SyncCoordinator {
  SyncCoordinator({
    required RunSyncUseCase runSyncUseCase,
    required GetSyncStateUseCase getSyncStateUseCase,
    required SaveSyncStateUseCase saveSyncStateUseCase,
    required AuthRepository authRepository,
    FirebaseInstallations? installations,
  }) : _runSyncUseCase = runSyncUseCase,
       _getSyncStateUseCase = getSyncStateUseCase,
       _saveSyncStateUseCase = saveSyncStateUseCase,
       _authRepository = authRepository,
       _installations = installations ?? FirebaseInstallations.instance;

  final RunSyncUseCase _runSyncUseCase;
  final GetSyncStateUseCase _getSyncStateUseCase;
  final SaveSyncStateUseCase _saveSyncStateUseCase;
  ...
  void setAuthenticated(bool isAuthenticated) {
    ...
    TalkerService.instance.info('syncTag auth changed: $_isAuthenticated');
    if (_isAuthenticated) {
  ...
  Future<void> _runSync(String reason) async {
    ...
    final result = await _runSyncUseCase(deviceId: deviceId);
    result.fold(
      ifLeft: (failure) =>
          TalkerService.instance.error('syncTag sync failed', failure),
      ifRight: (_) => TalkerService.instance.info('syncTag sync success'),
    );
  ...
  Future<void> _refreshAuthState() async {
    ...
    TalkerService.instance.info('syncTag auth refreshed: $_isAuthenticated');
  }

  Future<String?> _ensureDeviceId() async {
    final stateResult = await _getSyncStateUseCase();
    SyncStateEntity? state;
    stateResult.fold(ifLeft: (_) {}, ifRight: (right) => state = right);
    ...
    final updated =
        (state ?? const SyncStateEntity(deviceId: null, lastCursor: 0))
            .copyWith(deviceId: id);
    await _saveSyncStateUseCase(updated);
    return id;
  }
}
